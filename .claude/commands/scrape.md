...
4. **Create the job with EMAIL EXTRACTION ON** (`"email": true` in the body — emails are the #1 lead field). Required fields — `keywords` (location baked into each term), `lat`/`lon` (strings), `max_time` (**nanoseconds** — seconds × 1e9; default `300000000000` = 300s), `depth` (the number in the query, else `5`). Easiest path: `py -3 scripts/scrape.py "<keyword>" --city "<City, ST>" --depth <n>` (Windows; use `python3` on macOS/Linux) (emails are already on by default; append `--socials` if the user said yes in step 3).
