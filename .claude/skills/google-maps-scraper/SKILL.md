...
- `max_time` — **NANOSECONDS**, not seconds. The API decodes this into a Go `time.Duration`, which
  reads a bare JSON number as nanoseconds. **Multiply your seconds by 1e9**: 300s → `300000000000`.
  Sending `300` means 300 *nanoseconds*; the server then silently floors the deadline at its **180s
  minimum**, so every job gets 180s no matter what you asked for. See "Truncated results" below.
...
curl -s -X POST http://localhost:8080/api/v1/jobs \
  ...
  -d '{"name":"job","keywords":["coffee shops in Austin TX"],"lang":"en","zoom":15,
       ...
       "depth":5,"email":true,"max_time":300000000000}'   # 300s × 1e9
...
The response field is `"Status"` (capital S): `pending` → `working` → `ok` or `failed`.

> ⚠️ **`ok` does NOT mean complete.** Upstream sets `failed` only when the job could not *start*.
> A job that hits its time budget is **cancelled and still reported as `ok`**, returning a partial CSV.
> Always run the truncation check below before telling the user how many businesses they got.
...
### ⚠️ Truncated results — check this on every job
The job deadline comes from `max_time` (**nanoseconds**), floored at **180s** by the server. When the
deadline hits, the scrape is cut off mid-run, the status flips to **`ok`**, and you get however many
rows finished — with no error anywhere.

**The only signal is elapsed time.** Record the time when you create the job; when it reports `ok`,
compare against the budget (`max(180, max_time_seconds)`):

- `elapsed >= budget - 15s` → **treat the results as INCOMPLETE.** Say so plainly: *"this hit its
  time limit and was cut off — these N rows are partial, not the full result set."* Then offer to
  re-run with a larger `max_time`, a lower `depth`, or the keywords split across runs.
- `elapsed` well under budget → the job finished on its own; the results are complete.

`scripts/scrape.py` does this automatically and prints an `INCOMPLETE` warning.
`scripts/test_scrape.py` is an offline self-check for the unit conversion and this rule.
**Never report a row count as final without checking.**
...
said). If yes → **use the script:** `py -3 scripts/scrape.py … --socials`.
...
> **Interpreter:** use `python3` on macOS/Linux and **`py -3` on Windows**. Windows' `python3` is a
> Microsoft Store stub that prints an ad and **exits 0**, so it looks like it worked — it did not.

**Shortcuts (prefer these for common cases):**
- One keyword: `py -3 scripts/scrape.py "<keyword>" <lat> <lon> [--depth N]` — create→poll→download in one go.
- Auto-geocode (no coords): `py -3 scripts/scrape.py "<keyword>" --city "<City, ST>" [--depth N]` — resolves
...
- **Batch (many keywords, ONE job):** `py -3 scripts/scrape.py --keywords-file <file> --city "<City, ST>"`.
...
- **Never expose** the API beyond `localhost` without an auth proxy. The scraper has no auth of its
  own — the `127.0.0.1` port binding is the only boundary, so do not change it.
...
## Troubleshooting
- `422 missing max time` → add `max_time` (**nanoseconds** — seconds × 1e9). `422 missing geo coordinates`
  → add string `lat`/`lon` (only enforced when `fast_mode:true`, but always send them).
- Stuck `working` → lower `depth` / raise `max_time` / IP throttled (add proxies or wait).
- **Fewer rows than expected, status `ok`** → the job hit its deadline. See "Truncated results".
