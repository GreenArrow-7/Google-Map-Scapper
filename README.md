...
- 🛠️ **Standalone script** — `scripts/scrape.py` (Python, stdlib only, no dependencies) if you'd rather not use Claude.
...
py -3 scripts/scrape.py "coffee shops in Austin TX" --city "Austin, TX"   # macOS/Linux: python3
...
├── .gitignore
├── scripts/
│   ├── scrape.py        ← the scraper: single, batch, auto-geocoding (stdlib only)
│   └── test_scrape.py   ← offline self-check (max_time units + truncation)
├── examples/
