...
> - **Windows:** use `scripts/scrape.py` and run it with **`py -3`**. Do NOT use `python3` on Windows —
>   it is a Microsoft Store stub that prints an ad and exits 0 (so it looks successful but runs nothing).
>   Every `python3 ...` example below becomes `py -3 ...`.
...
```bash
python3 scripts/scrape.py   # Windows: py -3 scripts/scrape.py "gyms in Miami FL" 25.7617 -80.1918 --depth 5
```
...
```bash
python3 scripts/scrape.py   # Windows: py -3 scripts/scrape.py "cafes in Austin TX" --city "Austin, TX" --depth 5
```

**Scrape many queries at once** (one job, many keywords) from a file:
```bash
python3 scripts/scrape.py   # Windows: py -3 scripts/scrape.py --keywords-file examples/queries.example.txt --city "Denver, CO"
```
...
| `422 missing max time` | Job body needs `max_time` in **nanoseconds** (seconds × 1e9, e.g. `300000000000`). |
...
| Fewer rows than expected, job says `ok` | The job hit its time budget. A timed-out job is reported as `ok` with **partial** results — raise `--max-time`, lower `depth`, or split the keywords. |
...
- **Always** send `max_time` in **nanoseconds** (seconds × 1e9) and `lat`/`lon` (strings). The scripts do this for you.
